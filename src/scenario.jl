...
function run_model(
    ...
    for tstep::Int64 in 2:tf
        ...
        for idx in 1:n_biogroups
            growth_constraints[biogroup_masks[:, idx]] .= linear_extension_scale_factors(
                C_cover_t[:, :, biogroup_masks[:, idx]],
                vec_abs_k[biogroup_masks[:, idx]],
                biogrp_lin_ext[:, :, idx],
                _bin_edges,
                habitable_max_projected_cover[biogroup_masks[:, idx]]
            )
        end
        ...
        for i in habitable_loc_idxs
            timestep!(
                functional_groups[i],
                recruitment[:, i],
                biogrp_lin_ext[:, :, loc_biogrp_idxs[i]] .* growth_constraints[i],
                biogrp_survival[:, :, loc_biogrp_idxs[i]]
            )
